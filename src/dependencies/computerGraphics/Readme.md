...
### New: TrafficBase map support

The Flask API now exposes the `trafficBase` city model so you can render the static map (roads, obstacles, destinations) and watch traffic lights switch colors. The new endpoints are:

- `POST /init` → boots the `CityModel` (defaults to `2022_base.txt`).
- `GET /getMap` → returns roads, destinations, obstacles and traffic lights with their grid coordinates.
- `GET /update` → advances the simulation one step and returns the updated traffic light states.

The WebGL viewer (`visualization/index.html`) consumes these endpoints and colors the lights red/green as the model steps.
